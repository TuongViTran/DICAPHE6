...
class CoffeeShopController extends Controller
{
    ...
    public function destroy(CoffeeShop $coffeeshop)
    {
        ...
        $coffeeshop->reviews()->delete();
    
        // Xóa ảnh liên quan
        foreach (['cover_image', 'image_1', 'image_2', 'image_3'] as $imgField) {
            $imagePath = public_path('frontend/images/' . $coffeeshop->$imgField);
            if ($coffeeshop->$imgField && file_exists($imagePath)) {
                unlink($imagePath);
            }
        }
    ...
    {
        $shop = Coffeeshop::with('user')->findOrFail($id);
